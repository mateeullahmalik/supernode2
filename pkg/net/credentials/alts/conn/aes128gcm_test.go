...
func getGCMCryptoPair(key []byte, counter []byte, t *testing.T) (ALTSRecordCrypto, ALTSRecordCrypto) {
	client, err := NewAES128GCM(ClientSide, key)
	if err != nil {
		t.Fatalf("NewAES128GCM(ClientSide, key) = %v", err)
	}
	server, err := NewAES128GCM(ServerSide, key)
	if err != nil {
		t.Fatalf("NewAES128GCM(ServerSide, key) = %v", err)
	}
	// set counter if provided.
	if counter != nil {
		if CounterSide(counter) == ClientSide {
...
func debugGoroutines(t *testing.T) {
	t.Cleanup(func() {
		time.Sleep(500 * time.Millisecond) // Give some time for cleanup
		buf := make([]byte, 1<<16)
		n := runtime.Stack(buf, true)
		fmt.Printf("=== Active Goroutines After Test ===\n%s\n", buf[:n])
	})
}
...
func getGoroutineID() int {
	...
	// Stack trace starts like: "goroutine 18 [running]:"
	fields := bytes.Fields(buf[:n])
	id, _ := strconv.Atoi(string(fields[1])) // Extract the ID
	return id
}
...
func (s) TestAES128GCMEncrypt(t *testing.T) {
	for _, test := range []cryptoTestVector{
		...
		if CounterSide(test.counter) == ClientSide {
